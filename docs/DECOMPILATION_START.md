...
This document marks the transition from repository setup into active Pokémon Alpha Sapphire decompilation work.
...
Do not assume that a layout from Omega Ruby, X, Y, or another 3DS title is identical.
...
Pokémon Alpha Sapphire is reconstructed as its own verified target first. Cross-title comparison uses explicit buckets:
...
- Verified Pokémon Alpha Sapphire retail target manifest: pending a locally available source
