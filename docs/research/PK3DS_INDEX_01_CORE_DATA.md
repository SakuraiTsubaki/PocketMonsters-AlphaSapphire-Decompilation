...
This is the first source-level index of `kwsch/pk3DS` for Generation VI with **Alpha Sapphire / ORAS emphasis**. It records what the public implementation does; it is not direct observation of this repository's retail target because no local retail image is available.
...
For comparison, XY uses different numbers/paths for these major archives. Therefore the project must maintain separate XY and ORAS maps.
...
`Learnset6` uses 4-byte ordinary entries consisting of signed 16-bit move ID and signed 16-bit level, followed by a 32-bit `-1` sentinel in the writer. ORAS uses the same record model as XY but at a different logical GARC path.
...
XY instead uses 8-bit `Format` and 8-bit `Class` and does not read that extra field in the same place.

Shared fields include battle type, party count, four items, AI, unknown bytes, healer flag, money and prize. Party records contain IV byte, packed configuration/PID byte, 16-bit level/species/form, optional held item, and optional four 16-bit moves.

Alpha Sapphire trainer parsing must therefore not reuse an XY header layout unchanged.

## 8. Encounter handling — ORAS lead

pk3DS uses distinct wild-editor code paths:

- XY: `Gen6/XYWE.cs`
- ORAS: `Gen6/RSWE.cs`

The ORAS editor explicitly comments on differences from the older XY layout. Encounter and DexNav structures are reserved for the next dedicated audit.

## 9. Mega Evolution / Rayquaza lead

`Gen6/MegaEvoEditor6.cs` contains an ORAS-specific branch for species entry `384` (Rayquaza), warning that it uses a different activation rule and can Mega Evolve if it knows Dragon Ascent.

This is an implementation lead for the special-case rule, not yet a complete retail battle-engine reconstruction.
...
| ORAS branch behavior in GameConfig/trainer/personal handling | Public implementation evidence |
...
| Alpha Sapphire ↔ Omega Ruby equality for each mapped resource | Not established by this batch |
| Japan ↔ other-market equality | Unknown |
| retail byte/hash match | Unavailable |

## Next pk3DS batch

Audit ORAS wild/DexNav encounters, egg moves, Mega Evolution table, items, Maison, text/language indices, and ExeFS/CRO mappings. Each result must remain pinned to an upstream commit and be cross-checked against another public implementation where possible.
