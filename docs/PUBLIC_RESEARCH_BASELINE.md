...
No local Pokémon Alpha Sapphire ROM, cartridge dump, CIA/CCI, extracted RomFS, or extracted ExeFS is currently available to this project.

Accordingly, this repository must distinguish **publicly documented/reconstructed knowledge** from **facts verified directly against a Pokémon Alpha Sapphire dump**. Until a lawful source dump is available, exact hashes, offsets, archive contents, per-revision binary layouts, and rebuild equivalence remain unverified.
...
- Pokémon Alpha Sapphire file-by-file RomFS/ExeFS inventory verified from the retail title.
