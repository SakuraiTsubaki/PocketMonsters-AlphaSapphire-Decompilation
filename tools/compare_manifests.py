...
def main() -> int:
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("left",type=Path); p.add_argument("right",type=Path); p.add_argument("-o","--output",type=Path); a=p.parse_args()
    text=json.dumps(compare(json.loads(a.left.read_text(encoding="utf-8")),json.loads(a.right.read_text(encoding="utf-8"))),indent=2,ensure_ascii=False)+"\n"
